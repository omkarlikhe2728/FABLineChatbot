...
class TeamsService {
  ...
  async sendAdaptiveCard(activity, cardJson, context) {
    try {
      ...
      logger.debug(`Sending Adaptive Card via context.sendActivity()`);
      logger.debug(`Activity from: ${activity?.from?.id}`);
      logger.debug(`Conversation: ${activity?.conversation?.id}`);
      logger.debug(`Context Activity ID: ${context?.activity?.id}`);
      logger.debug(`Service URL (from context): ${context?.activity?.serviceUrl}`);
      logger.debug(`Service URL Valid Format: ${this.debugService?.validateServiceUrl(context?.activity?.serviceUrl) ? 'YES' : 'NO'}`);
      logger.debug(`Service URL Region: ${this.debugService?.getServiceUrlFormat(context?.activity?.serviceUrl)}`);
      logger.debug(`Adapter Type: ${this.adapter?.constructor?.name}`);
      logger.debug(`Auth Method: ${this.authMethod}`);
